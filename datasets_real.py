...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # rotate_right = rotate_left + 15 * random.randint(2, 4)
        # rotate_right = rotate_left + 5 * random.randint(2, 12)
        rotate_right = rotate_left + 60

        # ratio = (rotate_right - rotate_left) // 15
        # ratio = (rotate_right - rotate_left) // 5
        ratio = 12

        if rotate_right == 360:
            rotate_right = 0

        # rotate_target = rotate_left + 15 * random.randint(1, ratio - 1)
        # rotate_target = rotate_left + 5 * random.randint(1, ratio - 1)
        rotate_target = rotate_left + 5 * random.randint(1, 11)

        left_name = self.data_dir + '/train_data/img_' + str(x) + '_' + str(rotate_left) + '.png'
        right_name = self.data_dir + '/train_data/img_' + str(x) + '_' + str(rotate_right) + '.png'
        target_name = self.data_dir + '/train_data/img_' + str(x) + '_' + str(rotate_target) + '.png'
        mask_name = self.data_dir + '/train_mask/img_' + str(x) + '_' + str(rotate_target) + '.png'

        left_img = load_img(left_name, 1) # 600*400
